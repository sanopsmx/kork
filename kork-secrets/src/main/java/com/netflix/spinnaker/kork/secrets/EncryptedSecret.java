 ...
 *
 * <p>
 * Note: Valid param-names match the regex: `[a-zA-Z0-9]+` Note: secret-params may contain ':' Note:
 * `encrypted` cannot be a param-name Note: There must be at least one [param-name]:[param-value]
 * pair Named parameters are used to allow for adding additional options in the future.
 */
@EqualsAndHashCode
@NoArgsConstructor
@Log4j2
public class EncryptedSecret {
  ...
  private static final String ENCRYPTED_STRING_REGEX = ENCRYPTED_STRING_PREFIX + ENCRYPTED_TAG_REGEX;
  ...
  /**
   * Tries to parse the provided value as an EncryptedSecret if possible or returns an empty Optional
   * otherwise.
   */
  ...
  public static boolean isEncryptedSecret(String secretConfig) {
    return secretConfig != null && (matchesEncryptedStringSyntax(secretConfig) || matchesEncryptedFileSyntax(
      secretConfig
    ));
  }
  ...
  private static boolean matchesEncryptedFileSyntax(String secretConfig) {
    return secretConfig.toLowerCase().startsWith(ENCRYPTED_FILE_PREFIX.toLowerCase()) && secretConfig.matches(
