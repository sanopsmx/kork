...
public class CRLFHeaderTest {
   ...
   */
  /*
   * @Test public void clientTest() throws IOException { Socket socket = new Socket("127.0.0.1",
   * port); try { // set the buffer size to 1 to force a TCP segment at 1 byte
   * socket.setSendBufferSize(1); // set TCP_NODELAY to true which prevents tcp from buffering
   * requests socket.setTcpNoDelay(true); InputStream in = socket.getInputStream(); OutputStream out =
   * socket.getOutputStream();
   *
   * String requestAndHeaders = "GET /header/test HTTP/1.1" + CR + LF + "Host: " + HOST + ":" + port +
   * CR // Writes the // CR to the // socket and // is // received // by the // server + LF // This LF
   * causes a 400 + "User-Agent: spinnaker-test/1.0" + CR + LF + CR + LF;
   *
   * // write the first header out.write(requestAndHeaders.getBytes(StandardCharsets.UTF_8)); // force
   * a write out.flush(); BufferedReader lineReader = new BufferedReader(new InputStreamReader(in));
   *
   * String line = lineReader.readLine(); while (line != null) { if (line.startsWith(HTTP_1_1_PREFIX))
   * { int status = Integer.parseInt(line.substring(HTTP_1_1_PREFIX.length()).trim());
   * Assertions.assertEquals(2, status / 100, "non-2xx status code returned: " + status); } line =
   * lineReader.readLine(); } } finally { socket.close(); } }
   */
}
